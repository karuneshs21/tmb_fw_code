`timescale 1ns / 1ps
//------------------------------------------------------------------------------------------------------------------
// 1-to-2 De-multiplexer converts 80MHz data to 40MHz - CFEB Version requires IOB=true in ucf for rx ffs
//------------------------------------------------------------------------------------------------------------------
// 12/03/01 Initial
...
// 08/21/09 Add posneg
//------------------------------------------------------------------------------------------------------------------
	module x_demux_ddr_muonic_cfeb
	(
	clock,
	clock_iob,
	posneg,
...
// Delay data n-bx to compensate for osu cable error
	...
	always @(posedge clock) begin
	dly      <=  delay_is-1;		// Pointer to clct SRL data accounts for SLR 1bx latency
