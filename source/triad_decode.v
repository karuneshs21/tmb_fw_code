...
// 	and outputs 4 half-strip pulses
//
...
//------------------------------------------------------------------------------------------------------------------------
	module triad_decode(clock,reset,persist,persist1,triad,h_strip,triad_skip);
...
//------------------------------------------------------------------------------------------------------------------------
	module triad_decode(clock,reset,persist,triad,h_strip);
...
// Pulse-width persistence counters
	...
	else if (busy_hs[ihs])	width_cnt[ihs] = width_cnt[ihs]-1;	// decrement count down to 0
	ihs=ihs+1;
	end
	end
