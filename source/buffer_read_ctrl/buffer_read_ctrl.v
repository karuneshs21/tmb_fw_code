...
//------------------------------------------------------------------------------------------------------------------
// 	02/22/02 Initial
//	02/25/02 Invert fifo_wen
//	02/26/02 Pipeline delays for RAM readout
//	02/26/02 Fixed pretrig read address
//	02/27/02 Fixed tbin counter
//	02/28/02 Tuned RAM read data pipelines
//	02/28/02 Fixed tbin_done calculation
//	02/28/02 Fixed bank switch delay calculation
//	03/02/02 Changed fifo_rdata to non-tristate, added mux here
//	03/08/02 Increased header count bits
//	03/11/02 FFd fifo_rdata before layer mux, added s3 stage, increased header offset from 7 to 8
//	03/12/02 Removed debug ports
//	12/11/02 Multi-buffer write section
//	12/12/02 Dual ported buffer manager so tmb rejection deletes a buffer entry
//	12/16/02 Various fixes, added infer mux to buf_sm to stop warning message
//	12/17/02 Buffer available continuous output for pretrigger enable
//	02/19/03 Start local read
//	02/21/03 Changed reset FF to SRL, added prestore counter to prevent new triggers while storing raw hits
//	02/26/03 Removed FF on ready, its FFd in sequencer
//	02/27/03 No progress, delayed by OSU and Hauser
//	02/28/03 Advanced wr_buf_pretrig to switch buffers in minimum time, removed fifo_wadr FF for speed
//	03/01/03 Reduce busy time count to minimum needed to prestore raw hits before trigger, added turbo bypass
//	03/05/03 Change hold_fifo so sequencer sees buffers full for the next event, not the currrent event
//	03/10/03 Tuned header delay, clear prestore counter on wr_buf_reset for faster startup
//	03/11/03 Fix bufswitch delay > to >= so it works with 7 tbins after pretrig
//	05/12/04 Add hold_fifo output for RPC
//	05/13/04 Add first read addr out for RPC
//	05/18/04 Moved first read addr calc to rpc.v
//	09/11/06 Mod for xst compiler
//	09/21/06 More xst mods, add layer_id to output, but sump it in top level
//	09/22/06 Mod priority encoder to explice else-if structure
//	09/22/06 Remove count1s function on nbufs because it makes a combinatorial loop, apparent xst bug
//	10/05/06 Replace for-loops with while-loops for xst
//	08/23/07 Increase nhbits
//	09/17/07 Begin bufferless mods
//	09/19/07 More bufferless mods
//	09/25/07 Add rpc
//	09/26/07 Mods for cfeb and rpc to co-exist in same module
//	09/27/07 Unify name convention, remove header counter
//	09/28/07 Accelerate cfeb readout 1bx
//	10/01/07 Conform rpc section to match cfeb section timing
//	10/03/07 Remove fifo output stage ffs for speed
//	10/04/07 Subtract read address offset from fifo ram read address to compensate for pre-trigger latency
//	10/11/07 Delay cfeb adr increment by 2bx to fix early switch issue
//	10/12/07 Delay rpc  adr increment by 2bx to fix early switch issue, reduce 1st and last word marker delays 1bx
//	10/15/07 Add 2bx delay to slice_cnt to align bxn/tbin mux switching
//	11/02/07 Remove buffer status outputs
//	12/12/07 Move buffer control logic to a separate module
//	12/13/07 Rename from clct_fifo
//	04/18/08 Rename resync
//	04/22/08 Tune read_adr_offset for new pre-trigger state machine
//	04/24/09 Add recovery states
//	04/30/09 Add miniscope readout
...
// RPC Raw Hits FIFO RAM Ports
	//fifo_radr_rpc, //KSEDIT 2022 rem RPC
	//fifo_sel_rpc,
...
// Multiplex incoming RAM data from 5 CFEBs, delays 1bx
...
// Delay slice counter 2bx to compensate for RAM access and RPC slice mux
	...
	//srl16e_bbl #(1) usrlr5 (.clock(clock),.ce(1'b1),.adr(dly1),.d(rpc_slice_cnt),.q(rpc_slice_cnt_dly)); // KSEDIT May 2022 RPC rem
...
// Delay rpc, tbin, and slice markers 2bx to coincide with incoming RAM data
	...
	//srl16e_bbl #(MXRPCB) usrlr0 (.clock(clock),.ce(1'b1),.adr(dly1),.d(rpc_sel     ),.q(rpc_adr_dly ));// KSEDIT May 2022 RPC rem
	//srl16e_bbl #(MXTBIN) usrlr1 (.clock(clock),.ce(1'b1),.adr(dly1),.d(rpc_tbin_cnt),.q(rpc_tbin_dly));// KSEDIT May 2022 RPC rem

// Accelerate busy out to go high when rd_start arrives, then go low 1bx before end of readout
	wire rd_rpc_busy   = (read_rsm == rsm_read);
	wire rpc_busy_fast = rd_start_rpc || rd_rpc_busy || rpc_busy_dly;

	//srl16e_bbl #(1) usrlr2 (.clock(clock),.ce(1'b1),.adr(dly0),.d(rd_rpc_busy ),.q(rpc_busy_dly));// KSEDIT May 2022 RPC rem

// First frame valid 1bx after rd_start, last frame 1bx after busy goes down
	//srl16e_bbl #(1) usrlr3 (.clock(clock),.ce(1'b1),.adr(dly1),.d(rd_start_rpc),.q(rpc_first_frame));// KSEDIT May 2022 RPC rem
	//srl16e_bbl #(1) usrlr4 (.clock(clock),.ce(1'b1),.adr(dly1),.d(rpc_done    ),.q(rpc_last_frame ));// KSEDIT May 2022 RPC rem
...
// Accelerate busy out to go high when rd_start arrives, then go low 1bx before end of readout
	...
	//srl16e_bbl #(1) usrlm0 (.clock(clock),.ce(1'b1),.adr(dly0),.d(rd_mini_busy ),.q(mini_busy_dly));// KSEDIT May 2022 RPC rem

// First frame valid 1bx after rd_start, last frame 1bx after busy goes down
	//srl16e_bbl #(1) usrlm1 (.clock(clock),.ce(1'b1),.adr(dly1),.d(rd_start_mini),.q(mini_first_frame));// KSEDIT May 2022 RPC rem
	//srl16e_bbl #(1) usrlm2 (.clock(clock),.ce(1'b1),.adr(dly1),.d(mini_done    ),.q(mini_last_frame ));// KSEDIT May 2022 RPC rem
...
// Miniscope FIFO Read State Machine
	...
	*/
	//ksedit dont remove these two? -> read_rsm is only defined in commented portion. comment these two
